...
export class AgentWorker {
    ...
    constructor(agentId: string, concurrency = 2) {
    ...
    async start() {
        ...
        while (this.running) {
            try {
                const job: TradeRequest | null = await PortfolioRedis.dequeueAgentJob(this.agentId);
                if (!job) {
                    // No job, wait a bit
     ...
     */
    private async processJobWithRetry(job: TradeRequest, retries = 3, delay = 2000) {
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                logger.info(
                    { job, result },
                    `✅ Trade executed for ${job.userId}, attempt ${attempt}`
                );
                return result;
            } catch (err) {
                logger.warn(
                    { job, attempt, err },
                    `⚠️ Trade attempt ${attempt} failed for ${job.userId}`
                );
                if (attempt < retries) {
