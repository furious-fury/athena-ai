        ...
        if (user.scwOwnerPrivateKey) {
            ...
            if (!user.apiKey || !user.apiSecret || !user.apiPassphrase) {
                ...
                try {
                    ...
                    const tempClient = new ClobClient("https://clob.polymarket.com", 137, eoaWallet, undefined, useProxy ? 1 : 0, // 1 = Proxy, 0 = EOA
        ...
        try {
            ...
            if (marketRes.ok) {
                ...
                if (marketData.tokens && Array.isArray(marketData.tokens)) {
                    const tokenObj = marketData.tokens.find((t) => t.outcome.toUpperCase() === trade.outcome.toUpperCase());
                    if (tokenObj) {
                    ...
                    if (Array.isArray(tokenIds)) {
                        if (trade.outcome.toUpperCase() === "YES" && tokenIds.length > 0)
                            assetTokenId = tokenIds[0];
                        else if (trade.outcome.toUpperCase() === "NO" && tokenIds.length > 1)
                            assetTokenId = tokenIds[1];
                    }
                }
            }
            else {
