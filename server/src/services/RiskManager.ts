...
export class RiskManager {
     ...
     */
    static async validateTrade(request: TradeValidationRequest): Promise<RiskCheckResult> {
     ...
     */
    private static async checkCooldown(userId: string, cooldownSeconds: number): Promise<RiskCheckResult> {
     ...
     */
    private static async checkMarketExposure(
        userId: string,
        marketId: string,
        amount: number,
        ...
        if (newExposure > maxMarketExposure) {
            logger.warn(
                `Trade blocked for ${userId}: market ${marketId} exposure would be ${newExposure}, max is ${maxMarketExposure}`
            );
            return {
                allowed: false,
     ...
     */
    private static async checkTotalExposure(
        userId: string,
        amount: number,
        ...
        if (newTotalExposure > maxTotalExposure) {
            logger.warn(
                `Trade blocked for ${userId}: total exposure would be ${newTotalExposure}, max is ${maxTotalExposure}`
            );
            return {
                allowed: false,
     ...
     */
    static async recordTrade(userId: string): Promise<void> {
        const lastTradeKey = `user:${userId}:lastTrade`;
