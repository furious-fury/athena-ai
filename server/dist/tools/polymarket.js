 ...
 */
export const get_markets = async (limit = 20) => {
    try {
 ...
 */
export const get_active_events = async (limit = 20) => {
    try {
...
export const get_positions = async (userId) => {
    try {
...
const getPriceViaWS = (tokenId, side) => {
                ...
                for (const update of updates) {
                    if (update.event_type === "book" && update.asset_id === tokenId) {
                        ...
                        if (side === "BUY" && update.asks?.length > 0) {
                            const best = update.asks.reduce((p, c) => parseFloat(c.price) < parseFloat(p.price) ? c : p);
                            ...
                            const best = update.bids.reduce((p, c) => parseFloat(c.price) > parseFloat(p.price) ? c : p);
                            clearTimeout(timeout);
                            ws.close();
                            resolve(parseFloat(best.price));
                            return;
...
export const place_trade = async (trade) => {
    ...
    try {
        ...
        if (user.scwOwnerPrivateKey) {
            ...
            // Initialize Client
            clobClient = new ClobClient("https://clob.polymarket.com", 137, eoaWallet, {
                key: user.apiKey || "",
                secret: user.apiSecret || "",
                passphrase: user.apiPassphrase || ""
            }, useProxy ? 2 : 0, useProxy ? proxyAddress || undefined : undefined);
        }
        else {
                ...
                if (marketRes.ok) {
                    ...
                    if (md.clobTokenIds) {
                        ...
                        if (Array.isArray(tids)) {
                            // Map Yes/No
                            if (trade.outcome.toUpperCase() === "YES" && tids.length > 0)
                                assetTokenId = tids[0];
                            else if (trade.outcome.toUpperCase() === "NO" && tids.length > 1)
                                assetTokenId = tids[1];
                        }
                    }
                }
        ...
        if (!assetTokenId)
            assetTokenId = trade.marketId; // Fallback
        ...
        if (!finalPrice) {
            ...
            const wsPrice = await getPriceViaWS(assetTokenId, side);
            if (wsPrice) {
 ...
 */
export const get_trades = async (userId) => {
    try {
        const user = await prisma.user.findUnique({
            where: { id: userId },
            select: { walletAddress: true, scwAddress: true }
        });
        if (!user || (!user.scwAddress && !user.walletAddress))
            return [];
        ...
        if (user.scwAddress)
            ...
            addresses.push(user.walletAddress); // Fallback if no proxy
        const fetchForAddress = async (addr) => {
            try {
        ...
        const uniqueTrades = Array.from(new Map(allTrades.map((t) => [t.transactionHash || Math.random(), t])).values());
        // Sort descending
        uniqueTrades.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
 ...
 */
export const close_position = async (userId, marketId, outcome) => {
    try {
        ...
        const position = positions.find((p) => p.asset === marketId ||
            p.conditionId === marketId);
        if (!position) {
            console.error(`[CLOSE] ❌ Position NOT FOUND for MarketId: ${marketId}`);
            console.log(`[CLOSE] Available Assets:`, positions.map((p) => p.asset).join(", "));
        ...
        const clobClient = new ClobClient("https://clob.polymarket.com", 137, eoaWallet, {
            key: user.apiKey,
            secret: user.apiSecret,
            passphrase: user.apiPassphrase
        ...
        try {
            ...
            console.warn("[CLOSE] Failed to fetch tick size, using default 0.01");
        }
