...
export class AgentManager {
    private static activeLoops: Map<string, AgentLoop> = new Map();

    static async startAgent(userId: string, agentId: string): Promise<boolean> {
    ...
    static async stopAgent(agentId: string): Promise<boolean> {
        // Stop in DB
        await prisma.agent.update({
            where: { id: agentId },
            data: { isActive: false }
